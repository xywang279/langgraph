...
from .tools import calc, kb_search, multi_search, now, unstable
...
TOOLS = [calc, now, kb_search, multi_search, unstable]
...
def _get_spec(tool_name: str) -> ToolExecutionSpec:
    return TOOL_SPECS.get(
        tool_name,
        ToolExecutionSpec(name=tool_name),
)
...
def _tools_in_response(message: AnyMessage) -> List[str]:
...
def _run_tool_without_fallback(tool_name: str, args: Dict[str, Any], spec: ToolExecutionSpec) -> Dict[str, Any]:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        except Exception as exc:  # pragma: no cover - 示例逻辑
            latency = time.perf_counter() - started
            total_latency += latency
            last_error = repr(exc)
...
def tool_orchestrator(state: MessagesState) -> Dict[str, List[ToolMessage]]:
    ...
    parallel_tasks = [
        task for task in tasks if not task["spec"].exclusive
    ]
    serial_tasks = [
        task for task in tasks if task["spec"].exclusive
    ]
    ...
    for task in sorted(serial_tasks, key=key_fn):
        outcome = _execute_tool(
            task["call"]["name"], task["args"], task["spec"]
        )
        idx = task["call"]["index"]
        results[idx] = _build_tool_message(task["call"], outcome)
...
def agent(state: MessagesState) -> Dict[str, List[AnyMessage]]:
    ...
    if not any(getattr(msg, "type", None) == "system" and getattr(msg, "content", "") == SYSTEM_PROMPT for msg in messages):
